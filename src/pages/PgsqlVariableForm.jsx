...
export default function PgsqlVariableManager() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (error) {
      ...
      alert(`Variable ${editingId ? "mise à jour" : "créée"} avec succès.`);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white px-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl p-8 shadow-lg w-full max-w-3xl">
        ...
        <h1 className="text-2xl font-bold mb-6">Gérer les données variables</h1>

        <form onSubmit={handleSubmit} className="space-y-4 mb-8">
          <div>
            <label className="block text-sm font-medium mb-1">Nom de la variable</label>
            <input
              type="text"
              value={variableName}
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Requête SQL</label>
            <textarea
              rows="4"
              value={requestSQL}
        ...
        <h2 className="text-xl font-semibold mb-3">Variables existantes</h2>
        <div className="space-y-2">
          {variables.length === 0 && <p className="text-gray-400">Aucune variable trouvée.</p>}
          {variables.map((v) => (
            <div
              key={v.id}
